<template>
  <div
      class="min-h-screen bg-gradient-to-b from-orange-100 via-rose-50 to-white flex items-center justify-center px-4 py-4"
  >
    <div class="w-full max-w-4xl mx-auto flex flex-col gap-6">
      <!-- TOP: Section info / level panel -->
      <section class="space-y-6 max-w-2xl mx-auto text-left">
        <!-- Gamey header -->
        <div class="space-y-3">
          <div
              class="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-white/80 backdrop-blur border border-orange-200 shadow-sm"
          >
            <span
                class="inline-flex h-5 w-5 items-center justify-center rounded-full bg-orange-500 text-[11px] text-white animate-bounce"
            >
              🎯
            </span>
          ...
          <div class="space-y-1">
            <h2
                class="text-xl md:text-2xl font-semibold text-gray-900 leading-snug flex items-center gap-2"
            >
              <span>{{ currentSection?.title || 'Getting to know you' }}</span>
              <span class="text-lg" v-if="currentSectionEmoji">
                {{ currentSectionEmoji }}
              </span>
            </h2>
            <p
                class="text-sm text-gray-600 leading-relaxed min-h-[2.5rem]"
            >
        ...
        <div v-if="totalSections > 0" class="space-y-3">
          ...
          >
            <span
                class="font-semibold text-gray-700 flex items-center gap-1.5"
            >
          ...
          <!-- Progress bar -->
          <div
              class="h-2 w-full rounded-full bg-white/60 border border-gray-200 overflow-hidden shadow-inner"
          >
            <div
      ...
      <section class="flex flex-col items-center">
        ...
        >
          <div
              class="h-10 w-10 rounded-full border-2 border-dashed border-gray-300 animate-spin"
          />
        ...
        >
          <Transition name="slide-fade" mode="out-in">
            <div
                ...
                class="rounded-3xl border border-gray-200 bg-white/95 backdrop-blur p-6 md:p-8 shadow-[0_22px_50px_rgba(15,23,42,0.15)] space-y-6 transform transition-transform duration-200 hover:scale-[1.01]"
            >
              <!-- Question text -->
              <div class="space-y-2">
                <p
                    class="text-[11px] uppercase tracking-[0.18em] text-gray-400 font-semibold flex items-center gap-1.5"
                >
                  <span
                ...
                <h3
                    class="text-2xl md:text-3xl font-semibold text-gray-900 leading-snug flex items-start gap-2"
                >
              ...
              <!-- SCALE -->
              <div
                  v-if="currentQuestion.input_type === 'scale'"
              ...
              >
                ...
                >
                  <span>➡️</span>
                  <span>
                    Slide to choose your spot. We’ll auto-jump to the next
                    one.
                  </span>
                </p>
              </div>
              ...
              >
                <button
                    ...
                    :class="
                    answers[currentQuestion.answer_key] === true
                ...
                <button
                    ...
                    class="flex-1 px-4 py-3 rounded-2xl text-sm font-medium border transition transform hover:-translate-y-0.5"
                    :class="
                    answers[currentQuestion.answer_key] === false
              ...
              >
                <button
                    ...
                    :class="
                    answers[currentQuestion.answer_key] === opt
              ...
              <div v-else class="text-[11px] text-gray-400">
                This question type is not yet supported. (You unlocked a dev
                to-do 👀)
              </div>
            </div>
          </Transition>
...
const currentQuestion = computed(
    () => currentSectionQuestions.value[currentQuestionIndex.value]
)
const currentSectionDescription = computed(
    () => currentSection.value?.description ?? ''
)
const currentSectionEmoji = computed(
    () => currentSection.value?.emoji ?? ''
)
...
const isLastQuestionInSection = computed(
    () =>
        ...
        currentSectionQuestions.value.length - 1
)

const allSectionQuestions = computed(() =>
    sections.value.flatMap((s) => s.questions)
)
const totalQuestions = computed(
    () => allSectionQuestions.value.length
)

const answeredCount = computed(() => {
  const ans = answers.value || {}
  return allSectionQuestions.value.filter(
      (q) => ans[q.answer_key] !== undefined
  ).length
})
...
const sectionIndicators = computed(() => {
  ...
  return sections.value.map((s, index) => {
    ...
    const answered = s.questions.filter(
        (q: any) => ans[q.answer_key] !== undefined
    ).length
